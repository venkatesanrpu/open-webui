...
error_log("SYSTEM PROMPT:  " . $system_prompt);
    ...
    $payload = [
        ...
        'user_prompt' => "Generate exactly $questioncount $level MCQs about $agenttext. Return JSON only that matches the schema.",
        'stream' => false,
        'json_schema' => $schema,
        'options' => [
    ...
    $result = local_ai_functions_call_endpoint($agentkey, 'mcq_agent', $payload);
    ...
    $apimetadata['api_duration_ms'] = $apiduration;
    ...
    if (is_string($content)) {
        ...
        $clean_content = preg_replace('/^```(?:json)?\s*/i', '', trim($content));
        $clean_content = preg_replace('/```$/', '', trim($clean_content));

        $mcqraw = json_decode($clean_content, true);
